...
def get_f_dict(F_PATH):
    ...
    with open(F_PATH + os.sep + "train.group.csv") as f:
        reader = csv.reader(f)
        for row in reader:
            for i in range(1, len(row)):
                name_dict[row[i]] = int(row[0])
    class_num = int(row[0])
    return name_dict, class_num
...
def read_graph(F_NAME, FUNC_NAME_DICT, CLASS_NUM):
    graphs = []
    classes = []
    ...
    with open(F_NAME) as f:
        for line in f:
            ...
            label = FUNC_NAME_DICT[str(g_info['fname'])] - 1
